...
function initMaster_init_args(src: Master_init_args) {
  return (builder: Builder) => {
    let b_0 = builder;
    b_0.storeAddress(src.owner);
    b_0.storeRef(src.content);
...
const Master_errors: { [key: number]: { message: string } } = {
...
const Master_getters: ABIGetter[] = [
  {
...
const Master_receivers: ABIReceiver[] = [
...
export class Master implements Contract {
  static async init(owner: Address, content: Cell, max_supply: bigint) {
    return await Master_init(owner, content, max_supply);
  }

  static async fromInit(owner: Address, content: Cell, max_supply: bigint) {
    const init = await Master_init(owner, content, max_supply);
    const address = contractAddress(0, init);
    return new Master(address, init);
  }
  ...
  readonly abi: ContractABI = {
    types: Master_types,
    getters: Master_getters,
    receivers: Master_receivers,
